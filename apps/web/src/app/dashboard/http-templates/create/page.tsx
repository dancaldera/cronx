...
export default function CreateHttpTemplatePage() {
  ...
  const createTemplateMutation = useMutation({
    mutationFn: (data: any) => apiClient.createHttpTemplate(data),
    onSuccess: () => {
      router.push("/dashboard/http-templates");
    },
    onError: (error: any) => {
      console.error("Failed to create HTTP template:", error);
    },
  });
  ...
  const onSubmit = async (data: HttpTemplateFormData) => {
    try {
      ...
      await createTemplateMutation.mutateAsync(processedData);
    } catch (error) {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Create a reusable HTTP request configuration for your CRON jobs
        </p>
      </div>
        ...
        {createTemplateMutation.error && (
          <div className="rounded-md bg-red-50 dark:bg-red-900/20 p-4">
            <div className="text-sm text-red-700 dark:text-red-400">
              Failed to create HTTP template. Please check your inputs and try again.
            </div>
          </div>
        )}
      </form>
