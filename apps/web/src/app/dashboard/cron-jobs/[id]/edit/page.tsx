...
interface EditCronJobPageProps {
  params: { id: string };
}

export default function EditCronJobPage({ params }: EditCronJobPageProps) {
  ...
  const onSubmit = async (data: CronJobFormData) => {
    try {
      await updateCronJobMutation.mutateAsync(data);
    } catch (error) {
  ...
  if (jobError || !cronJobResponse?.data) {
    return (
      <div className="rounded-md bg-red-50 dark:bg-red-900/20 p-4">
        <div className="text-sm text-red-700 dark:text-red-400">
          Error loading CRON job. Please try again.
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Update your scheduled HTTP request automation task
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <span className="text-gray-500 dark:text-gray-400">Success Count:</span>
              <span className="ml-2 font-medium text-green-600 dark:text-green-400">
                {cronJob.successCount}
              </span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Failure Count:</span>
        ...
        <div className="flex items-center space-x-4">
          <button
            type="submit"
            disabled={isSubmitting || updateCronJobMutation.isPending}
            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSubmitting || updateCronJobMutation.isPending
              ? "Updating..."
              : "Update CRON Job"}
          </button>
          <Link
        ...
        {/* Error Display */}
        {updateCronJobMutation.error && (
          <div className="rounded-md bg-red-50 dark:bg-red-900/20 p-4">
            <div className="text-sm text-red-700 dark:text-red-400">
              Failed to update CRON job. Please check your inputs and try again.
            </div>
          </div>
        )}
      </form>
