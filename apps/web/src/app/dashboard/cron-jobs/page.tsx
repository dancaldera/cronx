...
import { PlusIcon, PlayIcon, PauseIcon, TrashIcon, PencilIcon } from '@heroicons/react/24/outline';
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {cronJobs?.data?.map((job: any) => (
                  <tr key={job.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 font-mono">{job.cronExpression}</div>
                      <div className="text-sm text-gray-500">{job.timezone}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(job.isEnabled)}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                      ...
                      >
                        {job.isEnabled ? <PauseIcon className="h-5 w-5" /> : <PlayIcon className="h-5 w-5" />}
                      </button>
                      <button
